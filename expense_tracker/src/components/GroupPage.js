...
export default function GroupPage() {
  ...
  const handleEqualshare = async () => {
    ...
    try {
      ...
      console.error('Error adding users in Group split equally:', error.message);
    }
  };
  ...
  const handlePercentageShares = async () => {
    ...
    try {
      ...
      console.error('Error adding users in Group with percentage shares:', error.message);
    }
  };
  ...
  const showTransactionDetails = (transaction) => {
    ...
    const confirmEdit = window.confirm(`${details}\n\nDo you want to edit this transaction?`);
  ...
  return (
    <div>
      <div>
        <button onClick={() => { setShowUser(true) }} className='btn_AddUser'>Add User</button>
        {showUser && (
          <ul className='UserList'>
            {users.map((user) => (
              <li key={user.Id} className='user'>
                ...
                <div onClick={handleAddUser} className='btn_Add'>Add</div>
              </li>
            ))}
          </ul>
        )}
      ...
      <div>
        <button onClick={() => { setshowTransaction(true) }} className='btn_AddTransaction'>Add transaction</button>
        {showTransaction && (
          <div>
            <label>
              Total amount:
            ...
            <label>
              Transaction Name:
              <input type='text' value={TransactionName}
            ...
            <div className="custom-dropdown">
              <button className="selected-option" onClick={toggleDropdown}>Share Type</button>
              {showDropdown && (
                <div className="dropdown-options">
                  <div onClick={()=>{onclickhandlerES()}} id='ES'>Equal Share</div>
                  <div onClick={()=>{onclickhandlerPS()}} id='PS'>Percentage Share</div>
                </div>
              )}
            </div>
            {ShareType === 'equal' && (
      ...
      <div>
        ...
        <ul className='users'>
          {GroupUsers.map((user) => (
            <li key={user.Id}  className='User'>
              {user.Name} - {user.Id};
              <div onClick={() => { delete_user(user.Id) }} className='delete'>X</div>
            </li>
          ))}
        </ul>
      </div>
          ...
          {AllTransactions.map((transaction) => (
            <li key={transaction.Id}
             ...
             className='transaction'>
              ...
              <div onClick={() => { delete_transaction(transaction.Id) }} className='delete'>X</div>
            </li>
          ))}
        </ul>
      </div>
